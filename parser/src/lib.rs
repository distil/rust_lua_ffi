...
pub fn extern_ffi_mod<'a>(
    krate: &'a ::syntax::ast::Crate) -> Option<&'a Vec<::syntax::ptr::P<::syntax::ast::Item>>> {
    ...
    krate.module.items
        ...
        .and_then(|item| {
            if let Mod(::syntax::ast::Mod{ inner: _, ref items, }) = item.node {
                Some(items)
            } else {
                None
            }
            ...
            if let ::syntax::ast::ItemKind::Use(ref view_path) = item.node {
                match view_path.node {
                    ...
                    ::syntax::ast::ViewPath_::ViewPathList(ref path, ref path_list_items) => {
                        let path = ::syn::parse_path(&path.to_string()).unwrap();
                        let path_list_items = path_list_items
                            .iter()
                            .map(|path_list_item| {
                                let rename = ::syn::parse_ident(
                                    &*path_list_item.node.rename.unwrap_or(
                                        path_list_item.node.name).name.as_str()).unwrap();
            ...
            let args: Vec<_> = args
                ...
                .map(|arg| {
                    ...
                    let typ = match arg.ty.node {
                        ::syntax::ast::TyKind::Rptr(_, ::syntax::ast::MutTy {
                            ref ty, mutbl: ::syntax::ast::Mutability::Immutable
                        }) => {
                            match ty.node {
                                ...
                                ::syntax::ast::TyKind::Slice(ref ty) => {
                                    if let ::syntax::ast::TyKind::Path(_, ref path) = ty.node {
                                        ...
                                        panic!("Slice: Function arguments can only be immutable reference or immediate")
                                    }
                                },
                                _ => {
                                    panic!("Reference: Function arguments can only be immutable reference or immediate")
                                }
                            }
                        },
                        ::syntax::ast::TyKind::Path(_, ref path) => {
                        ...
                        _ => panic!("Function arguments can only be immutable reference or immediate"),
                    };
                    Argument {
...
pub fn function_declarations(functions: &[Function], uses: &[::quote::Tokens]) -> ::quote::Tokens {
    let extern_c_ffi_functions = functions
        .iter()
        .map(|function|{
            let argument_declaration = function.args
                .iter()
                .map(|arg| {
                    let ident = &arg.ident;
                    let typ = &arg.typ;
            ...
            let argument_passing = function.args
                .iter()
                .map(|arg| {
                    let ident = &arg.ident;
                    let typ = &arg.typ;
